...
import { Plus, Search, AlertTriangle, RefreshCw, ArrowBigLeft } from "lucide-react"
...
export default function HospitalPatientsPage({ monitorId, onClose, onAssignDevice }: { monitorId: string, onClose: () => void, onAssignDevice: (patientId: string) => void }) {
    ...
    const hancleDeviceAssignment = async (patientId: string) => {
        onAssignDevice(patientId);
    }
                                        ...
                                        <TableRow key={patient.id} className="group">
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    {monitorId != null ? (
                                                        <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => hancleDeviceAssignment(patient.id)}>
                                                            Assign to {monitorId}
                                                        </Button>
                                                    ) : null}
                                                    {/* Uncomment and add more buttons if needed */}
